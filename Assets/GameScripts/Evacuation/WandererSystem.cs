...
{
    [SerializeField] private string wandererId;
    ...
    public void SetWanderersPath()
    {
        ...
        for (int i = 0; i < wanderTargets.Length; i++)
        {
            ...
            wanderTargets[i].SetPath(wanderPaths[index], wandererId);
        ...
        for (int i = 0; i < obstacleTargets.Length; i++)
        {
            ...
            while (currentPathIndexes.Contains(index));
            obstacleTargets[i].SetPath(obstaclePaths[index], wandererId);
