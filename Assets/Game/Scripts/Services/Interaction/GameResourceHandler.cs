...
public class GameResourceHandler : MonoBehaviour
{
    ...
    {
        playerPoints = 100;// PlayerPrefs.HasKey("INT_PlayerPoints") ? PlayerPrefs.GetInt("INT_PlayerPoints") : playerPoints;
        linkCharges = 5;// PlayerPrefs.HasKey("INT_LinkCharges") ? PlayerPrefs.GetInt("INT_LinkCharges") : linkCharges;

        bondLength = true; //PlayerPrefs.HasKey("BOOL_BondLength") ? (PlayerPrefs.GetInt("BOOL_BondLength") == 1) : false;
        grabSpeed = true;// PlayerPrefs.HasKey("BOOL_GrabSpeed") ? (PlayerPrefs.GetInt("BOOL_GrabSpeed") == 1) : false;
